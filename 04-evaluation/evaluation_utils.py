...
def llm_structured_completion(client, instructions, user_prompt, output_type, model="gpt-5.4-mini"):
    import json
    ...
    messages = [
        {"role": "developer", "content": instructions + json_instructions},
        {"role": "user", "content": user_prompt}
    ]
    ...
    if hasattr(output_type, "model_validate_json"):
        parsed_object = output_type.model_validate_json(content)
    else:
        parsed_object = output_type.parse_raw(content)

    return parsed_object, response.usage
...
def llm_structured_retry(
    ...
    for attempt in range(max_retries):
        try:
            return llm_structured_completion(
                client,
                instructions,
                user_prompt,
                output_type,
